...
let JwtAuthGuard = JwtAuthGuard_1 = class JwtAuthGuard extends (0, passport_1.AuthGuard)('jwt') {
    ...
    canActivate(context) {
        ...
        this.logger.debug(`Verificando acesso: rota é pública? ${isPublic ? 'Sim' : 'Não'}`);
        if (isPublic) {
            return true;
        }
        const request = context.switchToHttp().getRequest();
        const token = request.headers.authorization;
        this.logger.debug(`Verificando token: ${token ? 'Token presente' : 'Token ausente'}`);
        return super.canActivate(context);
    }
    handleRequest(err, user, info) {
        if (err || !user) {
            this.logger.error(`Erro de autenticação: ${err?.message || 'Usuário não encontrado'}`);
