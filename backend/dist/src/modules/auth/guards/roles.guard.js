...
let RolesGuard = RolesGuard_1 = class RolesGuard {
    ...
    canActivate(context) {
        ...
        const { user } = request;
        if (!user || !user.role) {
            throw new common_1.ForbiddenException('User role is undefined');
        }
        ...
        if (!hasRequiredRole) {
            throw new common_1.ForbiddenException(`Access denied: Role '${user.role}' is not authorized for this resource`);
        }
