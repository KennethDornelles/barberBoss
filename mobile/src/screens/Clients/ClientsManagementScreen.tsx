...
// Mock de Histórico
const MOCK_HISTORY = [
    { id: '101', service: 'Corte + Barba', date: '20/12/2025', price: 'R$ 55,00' },
    ...
    const renderClientItem = ({ item }: { item: Client }) => (
        ...
        >
            <View style={styles.cardHeader}>
                ...
                <TouchableOpacity onPress={() => handleCall(item.phone)} style={styles.iconButton}>
                    <Ionicons name="call-outline" size={20} color="#3B82F6" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => openForm(item)} style={[styles.iconButton, { marginLeft: 8 }]}>
            ...
            <Modal visible={formVisible} animationType="slide" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        <Text style={styles.modalTitle}>{isEditing ? 'Editar Cliente' : 'Novo Cliente'}</Text>
                ...
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { height: '60%' }]}>
                        <View style={styles.historyHeader}>
                            <Text style={styles.modalTitle}>Histórico: {selectedClient?.name}</Text>
                                ...
                                clientHistory.map((hist) => (
                                    <View key={hist.id} style={styles.historyItem}>
                                        <View>
                                            ...
                                            <Text style={styles.historyDate}>{hist.startsAt ? new Date(hist.startsAt).toLocaleDateString() : '-'}</Text>
                                        </View>
                                        <Text style={styles.historyPrice}>{hist.status === 'COMPLETED' ? 'Concluído' : hist.status === 'CANCELED' ? 'Cancelado' : hist.status}</Text>
                                    </View>
                                ))
                            )}
                        </ScrollView>
...
const styles = StyleSheet.create({
    ...
    avatarContainer: { width: 45, height: 45, borderRadius: 22.5, backgroundColor: '#3B82F6', justifyContent: 'center', alignItems: 'center', marginRight: 12 },
    ...
    cancelButton: { flex: 1, padding: 14, marginRight: 10, borderRadius: 8, backgroundColor: '#374151', alignItems: 'center' },
    cancelButtonText: { color: '#FFF', fontWeight: '600' },
    saveButton: { flex: 1, padding: 14, marginLeft: 10, borderRadius: 8, backgroundColor: '#3B82F6', alignItems: 'center' },
